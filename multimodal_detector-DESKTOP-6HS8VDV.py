...
class MultimodalFallDetector:
    ...
    def __init__(self, 
        ...
        print("Initializing Audio Detector...")
        try:
            # Re-enable Audio (AST) as requested
    ...
    def _video_worker(self):
        ...
        while self.running:
            ...
            try:
                annotated_frame, fallen, state, method, _ = self.pose_detector.process_frame(frame)
    ...
    def _run_fusion_logic(self, timestamp):
        ...
        fused_score = (v_prob * w_v) + (a_prob * w_a)
        
        self.fusion_history.append(fused_score)
        
        # --- Temporal Confirmation (Sliding Window) ---
        # Require sustained high score for X seconds
