...
const App = () => {
  ...
  const [batchPath, setBatchPath] = useState("C:\\Users\\Abedi\\OneDrive - g.bracu.ac.bd\\fds\\test_videos");
  ...
  const startBatchProcessing = async () => {
    try {
      setError(null);
      const response = await axios.post(`${BACKEND_URL}/process_folder`, {
        folder_path: batchPath
      });
      setBatchJobId(response.data.job_id);
      setBatchStatus("initializing");
      pollBatchStatus(response.data.job_id);
    } catch (err) {
      setError("Failed to start batch processing: " + (err.response?.data?.error || err.message));
    }
  };
  ...
  const pollBatchStatus = useCallback(async (jobId) => {
    const poll = async () => {
      try {
        const response = await axios.get(`${BACKEND_URL}/job_status/${jobId}`);
        const status = response.data;
        setBatchStatus(status);

        if (status.status === 'completed') {
          setBatchResults(status.results);
          setBatchJobId(null); // Stop polling
    ...
    <>
      ...
      <div className="app-container">
        ...
        <main className="main-content">
          ...
          {/* --- Mode Toggle / Validation Section --- */}
          <div className="info-panel" style={{ borderLeft: "4px solid #2563eb" }}>
            ...
            <div style={{ display: 'flex', gap: '10px', alignItems: 'center', marginBottom: '10px' }}>
              ...
              />
              <button onClick={startBatchProcessing} disabled={!!batchJobId} className="btn btn-primary">
                {batchJobId ? "Processing..." : "Run Batch Test"}
              </button>
            </div>
            ...
            {batchStatus && (
              <div style={{ background: '#f8fafc', padding: '10px', borderRadius: '4px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.9rem', marginBottom: '5px' }}>
                  <span>Status: <strong>{batchStatus.status}</strong></span>
                  <span>Progress: {batchStatus.current} / {batchStatus.total}</span>
                </div>
                {batchStatus.current_file && <div style={{ fontSize: '0.8rem', color: '#666' }}>Current: {batchStatus.current_file}</div>}

                {/* Simple Progress Bar */}
                <div style={{ width: '100%', height: '8px', background: '#e2e8f0', borderRadius: '4px', marginTop: '5px' }}>
                  <div style={{
                    width: `${batchStatus.total ? (batchStatus.current / batchStatus.total) * 100 : 0}%`,
                    height: '100%',
                    background: '#2563eb',
                    borderRadius: '4px',
            ...
            {batchResults && (
              <div style={{ marginTop: '15px', maxHeight: '200px', overflowY: 'auto' }}>
                <h4>Results:</h4>
                {batchResults.length === 0 ? <p>No falls detected in this folder.</p> : (
                  <ul style={{ listStyle: 'none', padding: 0 }}>
                    {batchResults.map((res, i) => (
                      <li key={i} style={{ padding: '5px', borderBottom: '1px solid #eee' }}>
                        <strong>{res.file}</strong>: {res.events.length} Events
                        {res.events.map((e, j) => (
                          <span key={j} style={{ marginLeft: '10px', fontSize: '0.8rem', color: 'red' }}>
                            @{e.timestamp.toFixed(1)}s (Score: {e.score.toFixed(2)})
                          </span>
                        ))}
                      </li>
                    ))}
          ...
          <div className="control-bar">
            ...
            <div className="button-group">
              <button
                onClick={startCamera}
                className={`btn ${sourceType === "camera" && isProcessing
                  ? "btn-primary"
                  : "btn-secondary"
                  }`}
