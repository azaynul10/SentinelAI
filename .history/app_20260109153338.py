...
try:
    if os.path.exists('./checkpoints/fold_1_best/model_state.pt'):
        # Threshold set to 0.70 - Balanced between sensitivity and rejecting door slams
...
def detect_fall():
    ...
    try:
        ...
        # Process frame
        annotated_frame, fallen, state, method = pose_detector.process_frame(frame)
...
def get_replay():
    # Save replay to disk first
    output_dir = r'C:\Users\Abedi\OneDrive - Student Ambassadors\fds'
    replay_path = pose_detector.save_replay(output_dir)
    
    # Retrieve the last 15 seconds of frames for display
    try:
