...
class AdvancedPoseFallDetector:
    ...
    def _extract_raw_metrics(self, landmarks, shape):
        ...
        if self.prev_landmarks is not None:
            # Reconstruct previous landmarks safely
            # Note: In a real optimized system, we'd store prev CoM directly
            # For now, this assumes landmarks are valid
            pass 
            
        # Simplified speed calculation based on stored previous CoM would be better
        # But here we only have prev_landmarks object. 
        # Let's calculate current CoM speed if we have history.
        if self.metrics_buffer:
             # This is a simplification. Ideally, store prev_com as a class var.
             pass

        # Recalculate robust speed
        # To do this correctly without reconstructing everything, let's just store prev_com
        if not hasattr(self, 'prev_com'):
            self.prev_com = com
            
        dist = np.linalg.norm(com - self.prev_com)
    ...
    def get_replay_clip(self):
        with self.buffer_lock:
            # Return subsampled list for speed
