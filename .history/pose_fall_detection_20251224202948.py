...
class AdvancedPoseFallDetector:
    ...
    def __init__(self, model_path='xgb_final_model.json'):
        ...
        # Heuristic thresholds (tuned for webcam detection)
        self.VELOCITY_THRESHOLD = 150  # pixels/sec for rapid motion
        self.ANGLE_THRESHOLD = 45  # degrees from vertical
        self.ASPECT_RATIO_THRESHOLD = 1.2  # width/height when lying
    ...
    def _extract_metrics(self, landmarks, shape):
        ...
        if self.prev_com is not None:
            displacement = np.linalg.norm(com - self.prev_com)
    ...
    def _heuristic_fall_detection(self, metrics):
        ...
        if (rapid_motion and tilted) or (rapid_angle_change and tilted) or horizontal_body:
