...
class AdvancedPoseFallDetector:
    ...
    def __init__(self, model_path='xgb_final_model.json'):
        ...
        # 3. Buffers
        # CHANGED: Adjusted FPS to 10 to match React frontend interval (100ms)
        # CHANGED: Adjusted window_size to 15 (1.5s at 10fps) to match training time window
        self.fps = 10
        self.window_size = 15
        self.buffer_size = 150  # 15 seconds for replay
        ...
        self.ASPECT_RATIO_THRESHOLD = 0.9  # width/height (Reverted to 0.9 for better sensitivity)
        self.FALL_CONFIRM_FRAMES = 3  # (Reverted to 3 for faster detection)
    ...
    def _heuristic_fall_detection(self, metrics):
        ...
        # relaxed: Removed strict downward speed requirement for fall, but kept upward rejection above
        is_falling_motion = rapid_motion
        
        if (is_falling_motion and tilted) or (rapid_angle_change and tilted) or horizontal_body:
    ...
    def process_frame(self, frame):
        ...
        if results.pose_landmarks:
            ...
            # Heuristic detection (calculated every frame)
            # This is now used primarily as a valid flag for fallback, or to trigger ML checks
            heuristic_trigger = self._heuristic_fall_detection(metrics)
            
            # ML detection (uses sliding window)
            ml_fall = False
            ml_prob = 0.0
            
            # Decide whether to run ML inference:
            # 1. Model must be loaded
            # 2. Buffer must be full
            # 3. Trigger condition: Heuristic detected something suspicious
            # CHANGED: Removed periodic check to reduce False Positives. ML acts as a strict confirmation step.
            should_run_ml = (self.xgb_clf and 
                             len(self.metrics_buffer) == self.window_size and 
                             heuristic_trigger)
            ...
            if should_run_ml:
                ...
                if features is not None:
                    try:
                        ...
                        # Use strictly ML decision if available
                        # CHANGED: Increased threshold to 0.75 for higher precision
                        ml_fall = ml_prob > 0.75
                        
                        if not ml_fall and heuristic_trigger:
                             print(f"[REJECTED] Heuristic triggered but ML confidence low: {ml_prob:.2f}")

                    except Exception as e:
                        print(f"ML prediction error: {e}")
            
            # FINAL DECISION LOGIC
            # If we have the ML model, we IGNORE the heuristic trigger for the final alerting
            # to prevent false positives. We only trust the XGBoost model.
            # If we don't have ML, we fall back to heuristics.
            final_fall_decision = False
            
            if self.xgb_clf:
                if ml_fall:
                    final_fall_decision = True
            else:
                if heuristic_trigger:
                    final_fall_decision = True

            # Activate Fall State
            if self.fall_cooldown == 0:
                if final_fall_decision:
                    self.state = "FALLEN"
                    current_fall_detected = True
                    self.fall_cooldown = 90  # 3 second cooldown
                    print(f"[FALL DETECTED] Triggered by: {'ML' if self.xgb_clf else 'Heuristic'} (Prob: {ml_prob:.2f})")
                else:
                    self.state = "STANDING"
            ...
            # Show BUFFERING status on visual overlay
            visual_state = self.state
            if self.xgb_clf and len(self.metrics_buffer) < self.window_size:
                 visual_state = f"BUFFERING {len(self.metrics_buffer)}/{self.window_size}"
                 # Draw buffering bar or text
                 # White text with black background for readability
                 cv2.rectangle(annotated_frame, (5, 5), (300, 40), (0,0,0), -1) 
                 cv2.putText(annotated_frame, f"State: {visual_state}", (10, 30), 
        ...
        # Determine detection method for UI
        detection_method = "Hybrid ML" if self.xgb_clf else "Heuristic"
        # Overwrite state return if buffering (so UI badge shows it too, not just standing)
        returned_state = self.state
        if self.xgb_clf and len(self.metrics_buffer) < self.window_size:
             buff_msg = f"BUFFERING ({len(self.metrics_buffer)}/{self.window_size})"
             detection_method = buff_msg
             # We can optionally set the state to this string too, or keep it STANDING.
             # Setting it to buff_msg makes it very clear in the UI badge.
             if self.state == "STANDING":
                 returned_state = buff_msg

        return annotated_frame, current_fall_detected, returned_state, detection_method
